...
export class DetailComponent implements OnInit {
  ...
  onselectstart(date) {
    ...
    let start = date[0].toLocaleString('en-GB', {
      timeZone: 'UTC'
    });
    let end = date[1].toLocaleString('en-GB', {
      timeZone: 'UTC'
    });
    console.log(start);
    console.log(end);
  ...
  onselectend(date) {
    this.enddate = this.end.toLocaleString('en-GB', {
      timeZone: 'UTC'
    });

    console.log(this.end.toLocaleString('en-GB', {
      timeZone: 'UTC'
    }))
  ...
  reservation() {
    ...
    this.router.navigate(['/car/book'], {
      queryParams: {
        'st': this.startdate,
        'se': this.enddate,
        'searchid': this.res.id
      }
    });
  }
